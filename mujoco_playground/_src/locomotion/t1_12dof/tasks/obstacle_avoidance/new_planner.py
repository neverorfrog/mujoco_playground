...
class FootstepPlanner:
    ...
    def plan(
        ...
        start_times = jp.zeros(cfg.max_steps)
        ds_start_times = jp.zeros(cfg.max_steps)
        end_times = jp.zeros(cfg.max_steps)
        ...
        def timestep_iteration(i, state: PlannerState):
            ...
            def integrate_pelvis(state: PlannerState) -> PlannerState:
                ...
                jax.debug.print("Timestep {}: Command = ({:.3f}, {:.3f}, {:.3f})", i, command[0], command[1], command[2])
            ...
            def place_new_footstep(current_state: PlannerState):
                # jax.debug.print("  Placing footstep: {}", current_state.step_index)
                ...
                # jax.debug.print("   SS duration: {:.3f}, DS duration: {:.3f}", ss_duration, ds_duration)
                # jax.debug.print("   SS start: {:.3f}, DS start: {:.3f}, End time: {:.3f}", ss_start, ds_start, end_t)
                # jax.debug.print("   Is first step? {}", is_first_step)
                
                # Calculate new foot pose
                lat_sign = jp.where(s.swing_foot == Foot.LEFT, 1.0, -1.0)
                ...
                end_pose = jp.where(
                    is_first_step, 
                    jp.where(s.swing_foot == Foot.RIGHT, s.L, s.R), 
                    end_pose
                )
                start_pose = jp.where(s.swing_foot == Foot.RIGHT, s.R, s.L)
                support_pose = jp.where(s.swing_foot == Foot.RIGHT, s.L, s.R)
                
                # Update new foot pose
                next_L = jp.where(s.swing_foot == Foot.LEFT, end_pose, s.L)
                next_R = jp.where(s.swing_foot == Foot.RIGHT, end_pose, s.R)
                
                # Update swing foot
                next_swing_foot = 1 - s.swing_foot
                next_swing_foot = jp.where(is_first_step, s.swing_foot, next_swing_foot)
                
                # Update event-based arrays
                j = s.step_index
                swing_foot_ids_new = s.swing_foot_ids.at[j].set(s.swing_foot)
            ...
            # jax.debug.print("   Step index: {}", state.step_index)
            # jax.debug.print("   Time since last step: {:.3f} / {:.3f}", time_since_last_step_new, step_duration)
            # jax.debug.print("   Pelvis pos: ({:.3f}, {:.3f}), theta: {:.3f}", new_footstep_event.pelvis_pos[0], new_footstep_event.pelvis_pos[1], new_footstep_event.pelvis_theta)
            # jax.debug.print("   Foot L pos: ({:.3f}, {:.3f}), theta: {:.3f}", state.L[0], state.L[1], state.L[2])
            # jax.debug.print("   Foot R pos: ({:.3f}, {:.3f}), theta: {:.3f}", state.R[0], state.R[1], state.R[2])
            # jax.debug.print("   Is swing foot LEFT? {}", state.swing_foot == Foot.LEFT)
    ...
    def compute_zmp_trajectory(self, footstep_plan: FootstepPlan) -> ZMPTrajectory:
        ...
        L0 = footstep_plan.start_poses[0]
        R0 = footstep_plan.support_poses[0]
        ...
        jax.debug.print("\nFull ZMP trajectory computed.")
        jax.debug.print("   ZMP X: {}", zmp_x_full)
        jax.debug.print("   ZMP Y: {}", zmp_y_full)
        jax.debug.print("   ZMP Theta: {}", zmp_theta_full)
...
if __name__ == "__main__":
    ...
    fs_plan = planner.plan(
        jp.array([0.0, 0.1, 0.0]),
        jp.array([0.0, -0.1, 0.0])
    )
    zmp_traj = planner.compute_zmp_trajectory(fs_plan)
    
    # Plot with overlays
    map.plot_map_with_plan(fs_plan, zmp_traj, "map_with_footsteps.png")