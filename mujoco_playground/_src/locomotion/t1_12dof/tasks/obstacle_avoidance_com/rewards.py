...
class ObstacleAvoidanceRewards:
    ...
    def _reward_planner_com_vel_axis(
        ...
        """Track desired COM velocity from plan."""
        ref_com_vel = jax.lax.cond(
            axis == 0,
            lambda: info["ref_com_vel_x"][info["plan_timestep"]],
            lambda: info["ref_com_vel_y"][info["plan_timestep"]],
        )
        actual_vel = data.subtree_linvel[self.env._torso_body_id][axis]  # COM velocity
    ...
    def _reward_velocity_direction_alignment(
        self, command: jax.Array, local_linvel: jax.Array
    ) -> jax.Array:
        """
        Reward for moving in the same direction as the commanded velocity.
        Uses cosine similarity between command and actual velocity vectors.
        Returns 1.0 when perfectly aligned, 0.0 when perpendicular, -1.0 when opposite.
        Only active when command magnitude exceeds min_command_magnitude threshold.
        """
        # Extract x,y components
        cmd_xy = command[:2]  # [vx, vy]
        vel_xy = local_linvel[:2]  # [vx, vy]
        
        # Compute norms
        cmd_norm = jp.linalg.norm(cmd_xy) + 1e-8  # Add epsilon to avoid division by zero
        vel_norm = jp.linalg.norm(vel_xy) + 1e-8
        
        # Only compute alignment if there's a meaningful command
        # (avoid rewarding random movement when command is zero)
        has_command = cmd_norm > self.config.min_command_magnitude
        
        # Cosine similarity: dot product of normalized vectors
        cosine_sim = jp.dot(cmd_xy, vel_xy) / (cmd_norm * vel_norm)
        
        # Return alignment reward, but only if there's a command
        return jp.where(has_command, cosine_sim, jp.array(0.0))
